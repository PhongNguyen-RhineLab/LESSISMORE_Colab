...
class CubSubModularExplanationV2(object):
    ...
    def define_recognition_model(self, num_classes, pretrained_path):
        ...
        model = tf.keras.models.Model(inputs=model_base.input,
                                      outputs=[model_base.get_layer(layer_name).output, model_base.output])
    ...
    def __call__(self, image_set, id=None):
        ...
        self.saved_json_file["smdl_score_max_index"] = self.saved_json_file["smdl_score"].index(
            self.saved_json_file["smdl_score_max"])
